...
function onPointerUp(e){
  ...
  if(grpEl){
    ...
    if(itemObj) itemObj.location = 'group:' + grpEl.getAttribute('data-group');
  } else if(poolHit){
...
function assignGroupColors(){
  ...
  Array.from(groupGrid.children).forEach(el=>{
    ...
    el.style.color = '';
    el.style.border = '';
    const existingBadge = el.querySelector('.grp-b');
  ...
  const start = g * per;
  for(let i = start; i < start + per && i < selectedEls.length; i++){
...
// remaining selected (incomplete group) should not have badge or grouped class (already handled by reset)
}
